...
export default function ProductCard({
	...
	return (
		<Card className="max-w-[350px] pt-4 xl:max-w-[290px] border-0 rounded flex flex-col">
			<CardContent>
				<img className="aspect-square" onLoad={() => console.log('image loaded')} src={product.image} alt={`${product.title} image`} />
			</CardContent>
				...
				<div className="flex justify-between w-full">
					{"price" in product ?
						<ProductDialog
							...
							description={product.description ? product.description : "something"}
						/>
						: <PizzaDialog
							slug={product.slug}
							title={product.title}
						...
						onClick={() =>
							!added
								? addProduct({
									...
									description: product.description,
									price: price,
									size: "sizes" in product ? product.sizes[0].sizeTitle : undefined,
									quantity: 1,
								})
